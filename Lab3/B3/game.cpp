...
COLLIDE gameCheckCollide(GAME * g)
{
	...
	SDL_Rect p1 = { g->p1.x, g->p1.y, PADDLE_WIDTH,g->p1.height };
	SDL_Rect p2 = { g->p2.x, g->p2.y, PADDLE_WIDTH,g->p2.height };
	...
	if (rectCollide(&b, &p1)) {
		...
		g->p1.height = ((rand() % 10)+1) * BOARD_CELL;
		return HIT_VERT;
	}
	/* bóng va chạm với mặt trong ván bên phải */
	if (rectCollide(&b, &p2)) {
		g->b.x = g->p2.x - BALL_SIZE;
		g->p2.height = ((rand() % 10) + 1) * BOARD_CELL;
		return HIT_VERT;
	}
...
void gameUpdate(GAME * g)
{
	...
	case HIT_HORZ: 
		g->b.vy *= -1; 
		break; /* va chạm ngang: đảo vy */
	case HIT_VERT: 
		g->b.vx *= -1; 
		break; /* va chạm dọc: đảo vx */
	default: break;
	}
	/* tính điểm */
